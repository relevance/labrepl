...
(defn arg-type-preconditions-ins
  ...
  [[:h3 "arg-type-preconditions"]
   [:p "To build " (c defstrict) " we will start with a helper function " (c arg-type-preconditions) " that converts argument specifiers into precondition forms. For example:"
    (repl-code "(arg-type-preconditions '[^String s ^Integer i])") "We'll build it inside-out from the REPL."]
   [:ol
    [:li "First, write an " (c instance-check) " function that checks a symbol for type hints, and emits an " (c instance?) "check."
     (showme instance-check)]
    [:li "Test " (c instance-check) " from the repl:"
     (repl-code "(instance-check '^String s)")]
    [:li "Now you can write " (c arg-type-preconditions) ". It simply maps " (c instance-check) " over all the args, removes the nils, and puts the checks into the " (c {:pre []}) " structure of a Clojure precondition."
