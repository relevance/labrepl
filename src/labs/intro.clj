...
(defn overview
  []
  [[:h3 "Overview"]
   [:p "In this lab you will learn to call functions from the Clojure Read-Eval-Print Loop (REPL), and a set of tools that you can use from the REPL to explore Clojure. Note that some of these tools are not available with a minimal install of Clojure. Later in the course you will see how to include all of these tools in your own projects."]
   [:p "You need to be running the labrepl to perform the steps in this lab."]])
...
(defn documentation
  ...
  [[:h3 "Documentation"]
   [:ol
    ...
    [:li
     [:code "find-doc"]
     " can also handle regular expressions, which in Clojure look like strings prepended by an octothorpe. Use a regular expression to find all the " [:code "find-"] " functions."
     (code (find-doc #"find-\w+"))]
    [:li
     "There is no documentation like the source code. Use "
     [:code "source"]
...
(defn java
  ...
  [[:h3 "Java"]
   [:ol
    ...
    [:li
     "You can create Java instances with forms like "
     ...
     (code (java.util.Date.))]
    [:li
     "Once you have an instance you probably will want to call methods on it. For now, stuff your date into a global using "
     [:code "def"] ": "
