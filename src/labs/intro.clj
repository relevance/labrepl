    ...
    [:li "Find your labrepl window, which should have a prompt like "
     ...
     [:li "Function names do not have to be words. Try some math functions:"
      (code (+ 1 2)
            ...
            (/ 22.0 7)
            (+ 1 2 3 4 5))]
     [:li "You can define your own functions with "
      [:code "defn"]
...
(defn documentation
  ...
  [[:h3 "Documentation"]
   [:ol
    [:li "To learn about a function, you can examine its docstring. Print the docstring for "
     [:code "println"]
     " using the form"
     (code (doc println))]
    ...
    [:li
     [:code "find-doc"]
     " can also handle regular expressions, which in Clojure look like
     strings prepended by an octothorpe. Use a regular expression to
     find all the " [:code "find-"] " functions."
     (code (find-doc #"find-\w+"))]
    [:li "There is no documentation like the source code. Use "
     [:code "source"]
     " to view the soure code of a function."
     (code (source println))]
    [:li "Notice how the source for "
     [:code "println"]
     " includes a string before the arguments. This is how you define
      docstrings. Try it with your own functions: "
     (code (defn triple
...
(defn java
  ...
  [[:h3 "Java"]
   [:ol
    [:li "Clojure provides direct, unproxied access to Java. To call a static function use a form like "
     (code "(ClassName/function & args)")
     "(The "
     [:code "& args"]
     " is shorthand for zero or more arguments.) Try calling"
     (code (System/getProperty "java.home"))]
    [:li "You can create Java instances with forms like "
     (code (new ClassName & args))
     " or the terser, more idiomatic "
     (code (ClassName. & args))
     " Try making a date. "
     (code (java.util.Date.))]
    [:li "Once you have an instance you probably will want to call methods on it. For now, stuff your date
          into a global using "
     [:code "def"] ": "
    ...
    [:li "If you can't remember all the date methods off the top of your
          head, you can pass either a class or an instance to "
     [:code "javadoc"]
     (code (javadoc now)
           (javadoc java.util.Date))]
    [:li "Or, to simply list method signatures, use show."
     (code (show now)
           (show java.util.Date))]]])
