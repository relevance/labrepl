...
(defn overview
  []
  [[:h3 "Overview"]
   [:p "When you are getting started with Clojure, you may (briefly!) miss imperative, mutable loops. In this lab, you will see how to loop in Clojure, and why you won't need to do it very often. You will implement three functions:"
    [:ol
  ...
  [[:h3 "min"]
   [:ol
    ...
    [:li "Clojure also has exactly one primitive control flow construct:" (c loop) ". Alone, it would loop forever, so you typically combine it with " (c if) " to terminate the recursion. Implement a function " (c min-1) " that takes the minimum of a variable-length argument list. It should"
     ...
     (showme min-1)]
    [:li "As you probably guessed from the suffix " (c -1) ", we are going to implement " (c min) " again, and make it better. The first improvement will use destructuring. Consider the following two forms, which are roughly equivalent:"
     (repl-code (let [[x & more] [1 2 3]]
                  {:x x :more more}))
     (repl-code (let [coll [1 2 3]]
                  {:x (first coll) :more (next coll)}))
     "Notice how the destructuring gets rid of the intermediate binding to " (c coll) ", which we never really wanted anyway. It also gets rid of the calls to " (c first) " and " (c next) ". Create a " (c min-2) " that uses destructuring in a similar way to simplify " (c min-1) "."
     (showme min-2)]
  ...
  [[:h3 "zipm"]
   [:ol
    ...
    [:li "One option is to loop over three variables: the result map " (c m) " (which gets bigger), and the input sequences " (c ks) " and " (c vs) " (which get smaller). Create a " (c zipm-1) " that uses a loop and " (c assoc) " to build the result map."
     (showme zipm-1)]
    [:li "As with the " (c min) " example, the " (c zipm) " loop can be improved with destructuring. Create a " (c zipm-2) " that destructures into the key and value collections, avoiding all the calls to " (c first) " and " (c next) "."
     (showme zipm-2)]
    [:li "Create a " (c zipm-3) " that " (c reduce) "s over a function " (c assoc) "ing in a key/value pair."
     (showme zipm-3)]
    [:li "The " (c reduce) " version is simple already, but we can make it simpler by simply calling a collection constructor such as " (c hash-map) ":"
     ...
     (showme zipm-4)]
    [:li "A very useful constructor is " (c into) ", which \"pours\" one collection into another. If you had a sequence of key/value pairs, you could pour them " (c into) " a map. Here is one way to cconvert the two sequences into a sequence of pairs:"
     (repl-code (map vector [:a :b :c] [1 2 3]))
     "Create a " (c zipm-5) " that pours the key/values " (c into) " a map:"
     (showme zipm-5)]
    [:li "The best approach of all is to notice that what you are doing is common enough to be built into Clojure, as in this case with " (c zipmap) "."
...
(defn minmax
  []
  [[:h3 "minmax"]
   [:p "In both the previous examples, the " (c reduce) " version looks simpler to my eye than the looping versions. But " (c reduce) " isn't always better. In this step, we will implement " (c minmax) ", which returns a map containing both the min and max from a collection."]
   [:ol
    [:li "Create a " (c minmax-1) " using a loop. This will look almost like " (c min-2) " from the earlier step, except that it will loop over three variables instead of two:"
     [:ol
