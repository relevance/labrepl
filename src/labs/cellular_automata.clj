(ns labs.cellular-automata
  (:use [labrepl.util :only (c showme repl-showme repl-code)]
        [solutions.automaton :only (new-board with-coords active-neighbors brians-brain-rules)]))
...
(defn groundwork []
  [[:h3 "Groundwork"]
   [:ol
    [:li "First, let's define some constants: " (c dim-board) " sets the board to 90x90, " (c dim-screen) "
          sets the screen to 600x600, and " (c dim-scale) " expresses the ratio of screen to board."
          (showme dim-board)
          (showme dim-screen)
          (showme dim-scale)]
  ...
  [[:h3 "Swing GUI"]
   [:ol
    [:li "Time to bring the GUI to life. Create " (c launch-2) ", which is the
          ...
          for a " (c panel) " that proxies " (c JPanel) ". The proxy should implement " (c paint) "
          by calling a (yet-to-be-written) " (c render) " function."
          (showme launch-2)]
    ...
    [:li "Now define " (c render) ", which takes a graphics object, an image, and the board. It should:"
     [:ol
      ...
      [:li "Loop over all the cells in all the rows of " (c (with-coords board)) ",
            calling out to a (to-be-written) " (c (render-cell background-graphics cell)) " function."]
      [:li "Draw the image back into the real graphics object."]]
     (showme render)]
    [:li "Next, define " (c render-cell) ". It should scale the cell up to screen
          units using " (c dim-scale) ", set the color using the (to-be-written) " (c state->color) "
          function, and fill the cell on screen."
     (showme render-cell)]
  ...
  [[:h3 "Let 'Er Rip"]
   [:ol
    ...
    [:li "You are ready to go! " (c launch) " your automaton and watch it go. When you
          (or your CPU) grow tired, you can stop it by setting the returned atom to " (c nil) ",
          or by simply killing the JVM."]]])

(defn bonus []
  [[:h3 "Bonus"]
   [:li "Load your code with " (c *warn-on-reflection*) " set to true, and then add type
         hints until your code is non-reflective."]
   [:li "What other things might make this code faster. Think about it a bit, and then read
         about " [:a {:href "http://www.bestinclass.dk/index.php/2009/10/brians-transient-brain/"}
